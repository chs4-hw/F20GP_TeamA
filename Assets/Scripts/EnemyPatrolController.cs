...
public class EnemyPatrolController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        RaycastHit2D hitPlayerL = Physics2D.Raycast(transform.position, Vector2.left, 300.0f);
        if (hitPlayerL.collider != null)
        {
            if (hitPlayerL.collider.gameObject.tag == "Player")
            {
                rb.AddForce(new Vector3(-100.0f, 0.0f, 0.0f), ForceMode2D.Force);
            }
        }
        RaycastHit2D hitPlayerR = Physics2D.Raycast(transform.position, Vector2.right, 300.0f);
